<template>
  <div class="m-4 p-4 bg-white shadow rounded">
    ...
    <form @submit.prevent="onSubmit">
      ...
      <template v-if="form.imageUrl.val">
        <img
          ...
          class="w-32 h-32 object-cover border rounded-full"     @click="selectImage"
        />
      </template>
      <template v-else>
    ...
    <span v-show="form.imageUrl.errorMessage" class="text-red text-sm">{{ form.imageUrl.errorMessage
    }}</span>
    </div>
    <label
